...
struct TestSetupActorState {
    value: u32,
}

impl TestSetupActorState {
...
fn Setup() {
    let state = TestSetupActorState { value: 1 };
    let cid = state.save();
...
#[allow(non_snake_case)]
fn TestSetup() {
    let state = TestSetupActorState::load(&sdk::sself::root().unwrap());
    if state.value != 1u32 {
        sdk::vm::abort(
            ExitCode::USR_ASSERTION_FAILED.value(),
            Some("Setup was not called"),
        )
    }
}
